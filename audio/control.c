...
static gchar **avrcp_black_list = NULL;
...
int avrcp_register(DBusConnection *conn, const bdaddr_t *src, GKeyFile *config)
{
	...
	if (config) {
		...
		err = NULL;
		avrcp_black_list = g_key_file_get_string_list(config, "AVRCP",
				"MetaDataBlackList", NULL, &err);
		if (err) {
			DBG("audio.conf: %s", err->message);
			g_strfreev(avrcp_black_list);
			g_error_free(err);
		}
	}
...
static gboolean device_is_avrcp_black_listed(char *dev_name)
{
	int i;

	for (i = 0; avrcp_black_list && avrcp_black_list[i] != NULL; i++) {
		if (g_strcmp0(dev_name, avrcp_black_list[i]) == 0)
			return TRUE;
	}
	return FALSE;
}
...
static uint8_t get_avrcp_quirks(struct audio_device *dev)
{
	...
	if (g_str_equal(name, "Ford Audio") &&
		device_is_avrcp_black_listed(name)) {
